    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid sm:grid-cols-3 gap-6">
            <div class="bg-white border border-gray-200 p-8 text-center">
                <div class="w-14 h-14 bg-green-100 flex items-center justify-center mx-auto mb-4">
...
<section class="py-16 bg-green-700 text-white">
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div><div class="text-4xl md:text-5xl font-bold mb-2">50+</div><div class="text-green-200">Professional Staff</div></div>
            <div><div class="text-4xl md:text-5xl font-bold mb-2">25+</div><div class="text-green-200">Districts Served</div></div>
            <div><div class="text-4xl md:text-5xl font-bold mb-2">21+</div><div class="text-green-200">Years of Experience</div></div>
            <div><div class="text-4xl md:text-5xl font-bold mb-2">10K</div><div class="text-green-200">Satisfied Customers</div></div>
        </div>
    </div>
</section>
...
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-12">
            <p class="text-sm uppercase tracking-widest text-green-600 font-semibold">Our Team</p>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse($teamMembers as $member)
                <div class="border border-gray-200 p-6 text-center hover:shadow-md transition-shadow">
                    <div class="w-24 h-24 rounded-full bg-green-100 mx-auto mb-4 overflow-hidden">
                        @if($member->photo)
                            <img src="{{ Storage::url($member->photo) }}" alt="{{ $member->name }}" class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full flex items-center justify-center text-2xl font-bold text-green-600">{{ strtoupper(substr($member->name, 0, 1)) }}</div>
                        @endif
                    </div>
                    <h3 class="font-bold text-gray-900">{{ $member->name }}</h3>
                    <p class="text-green-600 text-sm font-medium">{{ $member->position }}</p>
                    @if($member->bio)
