...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
    <div class="flex items-center justify-between mb-5">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-0.5">User login activity and session records</p>
        </div>
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="text-left border-b border-gray-100">
                    ...
                    <th class="pb-3 px-4 font-semibold text-gray-500 text-[11px] uppercase tracking-wider">User Agent</th>
                    ...
                    <tr class="border-b border-gray-50 hover:bg-gray-50/80 transition-colors">
                        <td class="py-3 px-4">
                            <div class="flex items-center space-x-3">
                                <div class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-bold text-xs">{{ substr($log->user?->name ?? '?', 0, 2) }}</div>
                                <div>
                                    <span class="font-medium text-gray-900">{{ $log->user?->name ?? 'Deleted User' }}</span>
                                    <p class="text-xs text-gray-400">{{ $log->user?->email ?? '' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-3 px-4 text-gray-500 font-mono text-xs">{{ $log->ip_address ?? '—' }}</td>
                        <td class="py-3 px-4 text-gray-500 max-w-[300px] truncate text-xs">{{ $log->user_agent ?? '—' }}</td>
                        <td class="py-3 px-4 text-gray-500 whitespace-nowrap">{{ $log->created_at->format('d M Y H:i:s') }}</td>
                    </tr>
                @empty
                    <tr>
